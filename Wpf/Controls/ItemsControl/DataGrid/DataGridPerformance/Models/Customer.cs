...
namespace WPFGridsBenchmark
{
		...
		IEditableObject
	{
		...
		static string[] _lastNames =
			"Ambers|Bishop|Cole|Danson|Evers|Frommer|Griswold|Heath|Jammers|Krause|Lehman|Myers|Neiman|Orsted|Paulson|Quaid|Richards|Stevens|Trask|Ulam"
			   .Split('|');

		static string[] _countries =
			"China|India|United States|Indonesia|Brazil|Pakistan|Bangladesh|Nigeria|Russia|Japan|Mexico|Philippines|Vietnam|Germany|Ethiopia|Egypt|Iran|Turkey|Congo|France|Thailand|United Kingdom|Italy|Myanmar"
			   .Split('|');
		...
		[Display(Name = "Name")] public string Name { get { return string.Format("{0} {1}", First, Last); } }

		[Display(Name = "Country")] public string Country { get { return _countries[_countryID]; } }
		...
		[Display(Name = "Brother")] public string Brother { get { return _brother; } }

		[Display(Name = "Cousin")] public string Cousin { get { return _cousin; } }

		// ** utilities
		static string GetString(string[] arr) { return arr[_rnd.Next(arr.Length)]; }
		static string GetName() { return string.Format("{0} {1}", GetString(_firstNames), GetString(_lastNames)); }

		// ** static list provider
		public static ObservableCollection<Customer> GetCustomerList(int count)
		{
			var list = new ObservableCollection<Customer>();
			for (int i = 0; i < count; i++) { list.Add(new Customer(i)); }
		...
		Customer    _clone;
		public void BeginEdit() { _clone = (Customer) this.MemberwiseClone(); }
		...
		public void CancelEdit()
		{
			if (_clone != null)
			{
				...
				{
					if (p.CanRead && p.CanWrite) { p.SetValue(this, p.GetValue(_clone, null), null); }
				}
			}
		}
