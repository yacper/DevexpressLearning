...
using VisualStudioDockingCustomSerialization.Services;
using VisualStudioDockingCustomSerialization.ViewModels;
...
public class MainViewModel
{
    ...
    private void DockLayoutManagerOnDockItemRestored(object sender, ItemEventArgs e) { }
...
#region Layout
    ...
    void OnLoadLayout()
    {
        ...
        var l = JsonConvert.DeserializeObject<List<WorkspaceViewModelData>>(str);
        foreach (var p in l)
        {
            Type t = Type.GetType(p.ViewModelType);
            ViewModelSource.GetPOCOType(t);

            // 使用ViewModelSource<T>.Create()进行创建
            var createMethod = typeof(ViewModelSource<>)
                               .MakeGenericType(t)
            ...
            var workspace = (PanelWorkspaceViewModel)createMethod.Invoke(null, null);
            workspace.OnRestore(p.Data);
            Workspaces.Add(workspace);
        }

        SaveLoadLayoutService.LoadLayout(file);

//        SaveLoadLayoutService.DockLayoutManager.DockItemRestored += DockLayoutManagerOnDockItemRestored;
//        SaveLoadLayoutService.DockLayoutManager.DockItemActivated += DockLayoutManagerOnDockItemActivated;
//SaveLoadLayoutService.DockLayoutManager.DockItemClosed += DockLayoutManagerOnDockItemClosed;
    }
    ...
    void OnSaveLayout()
    {
        var r = SaveLoadLayoutService.SaveLayout();
        if (r != null)
        {
            // 

            var str = JsonConvert.SerializeObject(Workspaces.Select(p => p.ViewModelData));
            // 写入到文件
            File.WriteAllText($"{r}.data", str);
        }
    }
